...
function VehicleModal({ car, isOpen, onClose }: { car: typeof allCars[0] | null; isOpen: boolean; onClose: () => void }) {
  useEffect(() => {
    if (isOpen && window.innerWidth < 1024) { // lg breakpoint
      gsap.fromTo('.mobile-car-details', 
        { y: '100%' },
        ...
        <div className="mobile-car-details lg:hidden fixed inset-0 z-50 bg-white overflow-y-auto transform translate-y-full">
          ...
          <div className="p-6">
            ...
            <div className="flex flex-col gap-4">
              <Button 
                onClick={() => contactSeller(car)}
                className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white py-6 rounded-none uppercase tracking-wider flex items-center justify-center gap-2"
              >
                <MessageCircle size={20} />
                Contact Seller
              </Button>
              <Button 
                ...
                className="w-full bg-[#e93d3d] hover:bg-[#d13232] text-white py-6 rounded-none uppercase tracking-wider flex items-center justify-center gap-2"
              >
                <Phone size={20} />
                Make Enquiry
              </Button>
...
function ShowroomPage({ onBack, onCarClick }: { onBack: () => void; onCarClick: (car: typeof allCars[0]) => void }) {
  ...
  useEffect(() => {
    gsap.set('.showroom-card', { opacity: 0, y: 50 });
    gsap.to('.showroom-card', {
      opacity: 1,
      y: 0,
      stagger: 0.1,
      duration: 0.6,
          ...
          {filteredCars.map((car) => (
            ...
            >
              ...
              <div className="p-4 lg:p-6">
                ...
                <div className="flex items-center gap-3 lg:gap-4 text-xs lg:text-sm text-[#333]/60 mb-3">
                  <span>{car.year}</span>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2 lg:gap-0">
                  ...
                  <Button 
                    ...
                    variant="ghost" 
                    className="text-[#121212] hover:text-[#e93d3d] p-0 flex items-center gap-1 group/btn cursor-pointer self-start lg:self-auto"
                  >
                    <span className="text-sm lg:text-base">View Details</span>
                    <ArrowRight size={14} className="lg:w-4 lg:h-4 transition-transform group-hover/btn:translate-x-1" />
                  </Button>
                </div>
              </div>
            </div>
...
function InventorySection({ onCarClick }: { onCarClick: (car: typeof allCars[0]) => void }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set('.car-card', { opacity: 0, y: 80, rotateY: -15 });
      gsap.to('.car-card', {
        opacity: 1,
        y: 0,
        rotateY: 0,
      ...
      <div className="w-full px-6 lg:px-12">
        ...
        <div className="services-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            <div 
              key={index}
              className="service-card group p-8 bg-white/20 border border-white/30 rounded-lg hover:bg-white/30 hover:border-[#e93d3d]/50 transition-all duration-300"
            >
              <service.icon 
